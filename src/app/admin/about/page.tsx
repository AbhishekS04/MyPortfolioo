    ...
    try {
      ...
      for (const item of experience) {
        ...
        if (typeof item.id === "string" && item.id.includes("temp")) {
          const { data, error } = await supabase
            ...
            .single();
          if (error) throw error;
          // Immutably update the experience array with the real DB id
          setExperience((prev) =>
            prev.map((exp) =>
              exp.id === item.id ? { ...exp, id: data.id } : exp,
            ),
          );
        } else {
          const { error } = await supabase
      ...
      for (const item of education) {
        ...
        if (typeof item.id === "string" && item.id.includes("temp")) {
          ...
          setEducation((prev) =>
            prev.map((edu) =>
              edu.id === item.id ? { ...edu, id: data.id } : edu,
            ),
          );
        } else {
          const { error } = await supabase
      ...
      for (const item of skills) {
        ...
        if (typeof item.id === "string" && item.id.includes("temp")) {
          ...
          if (error) throw error;
          setSkills((prev) =>
            prev.map((sk) => (sk.id === item.id ? { ...sk, id: data.id } : sk)),
          );
        } else {
          const { error } = await supabase
            .from("about_skills")
      ...
      for (const item of interests) {
        ...
        if (typeof item.id === "string" && item.id.includes("temp")) {
          ...
          setInterests((prev) =>
            prev.map((int) =>
              int.id === item.id ? { ...int, id: data.id } : int,
            ),
          );
        } else {
          const { error } = await supabase
          ...
          <div className="p-6 md:p-8 bg-[#111] border border-white/5 rounded-[32px] space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input
                ...
                onChange={(v: string) =>
                  setGeneral({ ...general, full_name: v })
                }
              />
              <Input
                label="Role Title"
                value={general.role_title}
                onChange={(v: string) =>
                  setGeneral({ ...general, role_title: v })
                }
              />
            </div>
            ...
            <Input
              ...
              value={general.profile_image_url}
              onChange={(v: string) =>
                setGeneral({ ...general, profile_image_url: v })
              }
            />

            <TextArea
              label="Bio Description"
              value={general.bio_description}
              onChange={(v: string) =>
                setGeneral({ ...general, bio_description: v })
              }
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Input
                ...
                onChange={(v: string) =>
                  setGeneral({ ...general, contact_email: v })
                }
              />
              <Input
                ...
                onChange={(v: string) =>
                  setGeneral({ ...general, phone_number: v })
                }
              />
              <Input
                label="Location"
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {experience.map((item, idx) => (
              <div
                key={item.id}
                className="p-6 bg-[#111] border border-white/5 rounded-2xl flex flex-col gap-4 group"
              >
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {education.map((item, idx) => (
              <div
                key={item.id}
                className="p-6 bg-[#111] border border-white/5 rounded-2xl flex items-start gap-4"
              >
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Input
                    label="Degree"
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {skills.map((item, idx) => (
              <div
                key={item.id}
                className="p-4 bg-[#111] border border-white/5 rounded-2xl flex flex-col gap-3 relative group"
              >
                <button
                  onClick={() =>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  />
                  <Input
                    label="Shortcode (Eg. Ps)"
                    value={item.icon_name}
                    onChange={(v: string) => {
                      const f = [...skills];
                      f[idx] = { ...f[idx], icon_name: v };
                      setSkills(f);
                    }}
                  />
                </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {interests.map((item, idx) => (
              <div
                key={item.id}
                className="p-4 bg-[#111] border border-white/5 rounded-2xl flex items-center gap-3 relative group"
              >
