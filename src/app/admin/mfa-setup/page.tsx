    ...
    useEffect(() => {
        const setupMFA = async () => {
            try {
                ...
                setSetupError(err.message || "Failed to setup MFA. detailed error in console.");
            } finally {
                setIsLoading(false);
            }
        };
        ...
        <div className="min-h-screen bg-[#050505] text-white p-6 md:p-12 flex items-center justify-center">
            <div className="max-w-xl w-full space-y-8">
                ...
                <div className="space-y-2">
                    <Link href="/admin" className="inline-flex items-center text-sm text-white/40 hover:text-white transition-colors mb-4">
                        <ArrowLeft size={16} className="mr-2" /> Back to Dashboard
                    </Link>
                    <h1 className="text-3xl font-bold tracking-tight">Setup Two-Factor Auth</h1>
                    <p className="text-white/40">
                        Scan the QR code below with your authenticator app (e.g. Google Authenticator) to enable 2FA.
                    </p>
                </div>
                    ...
                    {setupError ? (
                        <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl text-center space-y-2">
                            ...
                            <pre className="text-left bg-black/50 p-4 rounded text-xs text-red-300 overflow-auto max-w-full whitespace-pre-wrap font-mono">
                                {typeof setupError === 'string' ? setupError : JSON.stringify(setupError, null, 2)}
                            </pre>
                            <p className="text-white/40 text-xs">Ensure "Multi-Factor Authentication" is enabled in your Supabase Project Settings.</p>
                        </div>
                    ) : (
                        <div className="flex flex-col items-center justify-center space-y-6 bg-white/5 p-8 rounded-2xl border border-dashed border-white/10">
                            {qrCodeUrl ? (
                                <div className="bg-white p-4 rounded-xl">
                                    <Image src={qrCodeUrl} alt="QR Code" width={180} height={180} className="rounded-lg mix-blend-multiply" />
                                </div>
                            ) : (
                            ...
                            <div className="text-center space-y-2">
                                <p className="text-xs uppercase tracking-widest text-white/40 font-bold">Manual Entry Key</p>
