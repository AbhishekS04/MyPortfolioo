...
import { Users, MapPin, Link as LinkIcon, Mail, Building, Twitter } from "lucide-react";
import { GitHubProfile, GitHubAchievement } from "@/lib/github";
import Link from "next/link";

export function GitHubProfileHeader({ profile, achievements }: { profile: GitHubProfile, achievements: GitHubAchievement[] }) {
    return (
        <motion.div
        ...
        >
            ...
            <div className="text-center md:text-left">
                ...
                <p className="text-xl text-white/50 font-normal">{profile.login} <span className="text-white/30">• he/him</span></p>
            </div>
            ...
            <div className="flex items-center justify-center md:justify-start gap-4 text-sm text-white/60">
                <div className="flex items-center gap-1 hover:text-blue-400 transition-colors cursor-pointer">
                    ...
                    <span className="text-white font-bold">{profile.followers.totalCount}</span> followers
                </div>
                <span>•</span>
                <div className="hover:text-blue-400 transition-colors cursor-pointer">
                    <span className="text-white font-bold">{profile.following.totalCount}</span> following
                </div>
            </div>
            ...
            <div className="flex flex-col gap-2 text-sm text-white/60">
                ...
                {profile.email && (
                    <a href={`mailto:${profile.email}`} className="flex items-center gap-2 hover:text-blue-400">
                        <Mail className="w-4 h-4" />
                        <span>{profile.email}</span>
                    </a>
                )}
                {profile.websiteUrl && (
                    <a href={profile.websiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-blue-400 truncate">
                        <LinkIcon className="w-4 h-4" />
                        <span>{profile.websiteUrl}</span>
                    </a>
                )}
                {profile.twitterUsername && (
                    <a href={`https://twitter.com/${profile.twitterUsername}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-blue-400">
                ...
                <div className="pt-6 border-t border-white/10">
                    ...
                    <div className="flex gap-2">
                        {achievements.map((badge) => (
                            <div key={badge.name} className="w-16 h-16 relative group cursor-help" title={`${badge.name}: ${badge.description}`}>
                                <Image
                                    src={badge.imageUrl}
                                    alt={badge.name}
                                    fill
