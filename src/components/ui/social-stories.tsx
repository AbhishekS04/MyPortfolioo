...
export function SocialStories() {
  ...
  const durationMs =
    dynamicDuration ?? (currentStory?.duration ?? defaultDuration) * 1000;
    ...
    const animate = () => {
      if (!isPaused && isMediaLoaded) {
        ...
        const progressPercent = Math.min(
          (progressRef.current / durationMs) * 100,
          100,
        );
  ...
  const isCurrentVideo = currentStory
                  ...
                  <div className="absolute inset-0 z-10 flex flex-col p-5 pointer-events-none">
                    ...
                    <div className="flex gap-1.5 mb-4">
                      {stories.map((story, idx) => (
                        <div
                          key={story.id}
                          className="h-0.5 flex-1 bg-white/30 rounded-full overflow-hidden backdrop-blur-sm"
                        >
                          <div
                            ref={
                              idx === currentIndex ? activeProgressBarRef : null
                            }
                    ...
                    <div className="flex items-end justify-between gap-4 pb-1 pointer-events-auto">
                      ...
                      >
                        <ArrowUpRight
                          className="w-5 h-5 group-hover/btn:rotate-45 transition-transform duration-300"
                          strokeWidth={2.5}
                        />
                      </a>
                    </div>
