...
if (!SUPABASE_KEY || SUPABASE_KEY === "dummy-key") {
    console.error("❌ NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY is missing or set to dummy value");
    process.exit(1);
}

console.log("📡 Supabase URL:", SUPABASE_URL);
console.log("🔑 Key format:", SUPABASE_KEY.startsWith("eyJ") ? "JWT (anon key)" : SUPABASE_KEY.startsWith("sb_publishable_") ? "Publishable key (new format)" : "Unknown format");
...
async function testConnectivity() {
    ...
    try {
        ...
        if (res.status === 200) {
            ...
            fail("REST API auth failed", `HTTP 401 — API key is invalid or expired. Key format: ${SUPABASE_KEY.substring(0, 15)}...`);
            return false;
        } else if (res.status === 404) {
            fail("REST API 404", `'projects' table may not exist. Run SQL migrations.`);
            return true; // API is reachable, just table missing
        } else {
            fail("REST API unexpected response", `HTTP ${res.status}: ${JSON.stringify(res.data).substring(0, 200)}`);
            return false;
        }
    } catch (err) {
        if (err.message === "TIMEOUT") {
            fail("REST API TIMEOUT",
...
async function testAuthEndpoint() {
    ...
    try {
        ...
        if (res.status === 200) {
            pass("Auth endpoint reachable", `Email auth: ${res.data?.external?.email ?? "unknown"}`);
        } else {
            fail("Auth endpoint", `HTTP ${res.status}: ${JSON.stringify(res.data).substring(0, 200)}`);
        }
    } catch (err) {
        if (err.message === "TIMEOUT") {
    ...
    for (const table of TABLES) {
        try {
            ...
            if (res.status === 200) {
                ...
                fail(`Table '${table}'`, `ACCESS DENIED (${res.status}) — RLS or key issue`);
            } else {
                fail(`Table '${table}'`, `HTTP ${res.status}: ${JSON.stringify(res.data).substring(0, 150)}`);
            }
        } catch (err) {
            if (err.message === "TIMEOUT") {
...
async function testProjectsColumns() {
    ...
    const criticalColumns = ["is_hidden", "slug", "featured", "is_coming_soon", "is_currently_working", "tech_stack", "image_url"];
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                if (errMsg.includes("does not exist") || res.status === 400) {
                    fail(`Column 'projects.${col}'`, `MISSING — run: ALTER TABLE projects ADD COLUMN IF NOT EXISTS ${col} ...`);
                } else {
                    fail(`Column 'projects.${col}'`, `HTTP ${res.status}: ${errMsg.substring(0, 150)}`);
                }
            }
        } catch (err) {
...
async function testGalleryImages() {
    ...
    try {
        const res = await supabaseRequest("gallery_images?select=*&order=display_order.asc&limit=10");
        if (res.status === 200 && Array.isArray(res.data)) {
            if (res.data.length === 0) {
                warn("Gallery images", "EMPTY — images won't show. Add data via admin or SQL.");
            } else {
                pass("Gallery images", `Found ${res.data.length} images`);
                res.data.forEach((img, i) => {
                    const urlPreview = img.image_url ? img.image_url.substring(0, 60) + "..." : "NULL";
                    console.log(`    ${i + 1}. ${img.alt_text || "(no alt)"} → ${urlPreview}`);
                });
            }
        } else {
...
async function testProjectsData() {
    ...
    try {
        // Raw project count
        const rawRes = await supabaseRequest("projects?select=id,title,slug,featured,is_hidden&limit=20");
        if (rawRes.status === 200 && Array.isArray(rawRes.data)) {
            if (rawRes.data.length > 0) {
                ...
                rawRes.data.forEach((p, i) => {
                    console.log(`    ${i + 1}. "${p.title}" [slug: ${p.slug || "NULL"}] [featured: ${p.featured}] [hidden: ${p.is_hidden}]`);
                });
            } else {
            ...
            fail("Projects query", `HTTP ${rawRes.status}: ${JSON.stringify(rawRes.data).substring(0, 200)}`);
            return;
        }

        // Works page query
        const worksRes = await supabaseRequest("projects?select=id,title&is_hidden=eq.false&order=display_order.asc&limit=10");
        if (worksRes.status === 200 && Array.isArray(worksRes.data)) {
            if (worksRes.data.length > 0) {
                pass("Works page query (is_hidden=false)", `${worksRes.data.length} visible`);
            } else {
                warn("Works page query", "0 results — is_hidden column missing or all hidden");
            }
        } else {
            fail("Works page query", `HTTP ${worksRes.status} — is_hidden column likely missing`);
        }

        // Featured projects
        const featuredRes = await supabaseRequest("projects?select=id,title&featured=eq.true&is_hidden=eq.false&limit=10");
        if (featuredRes.status === 200 && Array.isArray(featuredRes.data)) {
            if (featuredRes.data.length > 0) {
                pass("Featured projects query", `${featuredRes.data.length} featured visible`);
            } else {
...
async function testAuthLogin() {
    ...
    try {
        ...
        if (res.status === 400) {
            const msg = res.data?.error_description || res.data?.msg || res.data?.error || "";
            if (msg.toLowerCase().includes("invalid") || msg.toLowerCase().includes("credentials")) {
                pass("Auth login endpoint", "Working (rejected invalid creds as expected)");
            } else {
            ...
            warn("Auth login endpoint", `HTTP ${res.status}: ${JSON.stringify(res.data).substring(0, 200)}`);
        }
    } catch (err) {
        if (err.message === "TIMEOUT") {
...
async function testNextAppRunning() {
    ...
    await new Promise((resolve) => {
        const req = http.request(
            { hostname: "localhost", port: 3000, path: "/", method: "HEAD", timeout: 3000 },
            (res) => {
                if (res.statusCode >= 200 && res.statusCode < 400) {
                    pass("Next.js dev server", `Running on :3000 (HTTP ${res.statusCode})`);
                } else {
...
async function testMiddleware() {
    ...
    for (const v of requiredVars) {
        if (process.env[v] && process.env[v] !== "dummy" && !process.env[v].includes("dummy")) {
            pass(`ENV ${v}`, "set");
        } else {
